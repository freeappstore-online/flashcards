...
const cardRadius = "1.25rem";
const btnRadius = "0.75rem";
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        {editing ? (
          ...
          >
            ...
            <button type="button" style={btnMuted} onClick={() => { setEditing(false); setEditName(deck.name); }}>
              Cancel
            </button>
          </form>
        ) : (
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        {confirming ? (
          <span className="flex gap-2 items-center">
            <span className="text-sm" style={{ color: "var(--error)" }}>Delete this deck?</span>
            <button style={btnError} onClick={onDelete}>Yes, delete</button>
          ...
          {deck.cards.map((card) => (
            ...
            >
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button
                  style={{ ...btnMuted, padding: "0.25rem 0.5rem", fontSize: "0.75rem", color: "var(--error)" }}
                  onClick={() =>
                    onUpdate((d) => ({
                      ...d,
...
function StudyView({
  ...
  function handleAgain() {
    setFlipped(false);
    if (queue.length === 1) {
      // Only one card left, just stay
      setQueue([...queue]);
    } else {
      // Move to a random position in the back half
      const rest = queue.slice(1);
      const insertAt = Math.max(1, Math.floor(rest.length / 2) + Math.floor(Math.random() * Math.ceil(rest.length / 2)));
      const newQueue = [...rest];
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-sm" style={{ color: "var(--muted)" }}>
          {deck.cards.length - queue.length + 1} / {deck.cards.length} reviewed
        </span>
        <span className="text-sm" style={{ color: "var(--muted)" }}>
      ...
      >
        <div
          style={{
            height: "100%",
            width: `${((deck.cards.length - queue.length) / deck.cards.length) * 100}%`,
            background: "var(--accent)",
            borderRadius: "2px",
      ...
      >
        <div className={`fc-card ${flipped ? "flipped" : ""}`}>
          ...
          >
            <div>
              <div className="text-xs mb-2 uppercase tracking-wide" style={{ color: "var(--muted)" }}>
                Front
              </div>
              {current.front}
            </div>
      ...
      {!flipped ? (
        ...
        <div className="flex justify-center gap-3">
          <button style={btnError} onClick={handleAgain}>
            Again
          </button>
